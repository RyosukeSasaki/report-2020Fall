...
peaks, _ = find_peaks(amp[0:int(N/2)], height=peak_threshold)   #ピークのindex求める
...
with open(outdir + "/" + name + ".txt", 'w') as f:
    ...
    print("peaks:", file=f)
    for i in peaks:
        print("Frequency:", freq[i], "  Amplitude:", amp[i], "  Phase(degrees):", np.degrees(phase[i]), file=f)
